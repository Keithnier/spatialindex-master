 	 ...
 	 while (queue.size() != 0)
		{
		 ...
		 int i_start =0;
		 int i_end = 0;
		 if(first.kind == 0) { i_start = first.before+1; i_end = first.after;}
		 if(first.kind == 1) { i_start = Math.max(first.before+1, index0+1);  i_end = first.after;}
		 if(first.kind == 2) { i_start = first.before+1;  i_end = Math.min(index0, first.after);}
				...
				if(missing.contains(i)) {
					...
					if(numofmissing == 0) {
                        ...
                        for(int j = index0-1; j >= 0; j--){
                      	  ...
                      	  if( penalty < weights[j].w_penalty + penalty_alpha*(Math.max(0, rank - j- 1))/base_k) break;
                      	  rank--;
                      	  ...
                      	  //skip
                      	  if( penalty < weights[j].w_penalty + penalty_alpha*(Math.max(rank - topk, 0)/base_k) ) continue;
                        ...
                        for(int j = index0+1; j < weights.length; j++){
                      	  ...
                      	  if( penalty < weights[j].w_penalty + penalty_alpha*(Math.max(0, rank - weights.length + j))/base_k) break;
                      	  rank--;
